...
def main():
    ...
    if (os.path.exists(cppfile) and os.path.isfile(cppfile) and cppfile.split('.')[1] == 'cpp'):
        final = zipcpp(cppfile)
        print(open(final).read())
        os.remove(final)
    else:
        print(f"{cppfile} not supported yet ...")
...
def preprocess(cppfile: str, prepfile: str):
    ...
    with (
        ...
        debug(">Preprocess...".title())
        debug(">>include no-standard headers + format...".title())
        ...
        concat(cppfile)
        tempstream.seek(0)
        debug(">>macro substitution + del comments...".title())
        ret = subprocess.run(
            "clang++ -E -xc++ -".split(),
            stdin=tempstream,
            stdout=prepstream,
        ...
        debug(f">Preprocess...done:".title(), prepfile, "\n")

def astdump(prepfile: str, astfile: str):
    with (
        open(astfile, 'w+') as w,
    ):
        debug(">Generate AST...".title())
        ret = subprocess.run(
        ...
        # if ret.returncode:
        #     exit(ret.returncode)
        debug(f">Generate AST...done:".title(), astfile, "\n")
...
def prune(astfile: str, prepfile: str, prunfile: str):
    ...
    debug(">Pruning...".title())
    debug(">>founding declarations...".title())
    decls = {}
    def visit_decls(x):
    ...
    for x in decls:
        debug('>>>', x.rjust(20), ':', decls[x][:-1])

    debug(">>founding references...".title())
    adj = {}
    ...
    debug(">>removing useless declarations...".title())
    usefuldecls = set()
    ...
    debug(">>removing blank lines...".title())
    ...
    for declid in alldecls:
        ...
        if declid in usefuldecls:
            while curline < seg.stop:
                ...
                curline += 1
        curline = seg.stop
    debug(">>validate final result...".title())
    ...
    debug(f">Pruning...done:".title(), prunfile, "\n")
