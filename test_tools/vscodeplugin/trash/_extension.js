...
const server = http.createServer((request, response) => {
    if (request.method == 'POST') {
        ...
        request.on('end', async () => {
            ...
            var msg = '';
            if (result.code == 0)
                msg = `New Task: ${result.name}`;
            if (result.code == 1)
                msg = `Duplicated Task: ${result.name}`;
...
const port = 12345;
server.listen(port);
vscode_1.window.showInformationMessage(`xtask Server is running at http://127.0.0.1:${port}/`);
const taskData = new WeakMap();
let xtaskStatusBarItem;
let xtestStatusBarItem;
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('xtask.liststash', async () => {
        ...
        async function get_items() {
            return (await xtasks.db.all('SELECT id, name, result FROM xtask WHERE stash != 0 order by name')).map((task) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('xtask.listdb', async () => {
        ...
        async function get_xtasks() {
            return (await xtasks.db.all('SELECT id, name, description, result, ctime FROM xtask order by mtime desc')).map((task) => {
        ...
        quickPick.onDidTriggerButton(async (e) => {
            switch (e.tooltip) {
                case 'Delete':
                    ...
                    if (result === 'Yes') {
                        await Promise.all(quickPick.selectedItems.map(item => xtasks.delete_xtask(taskData.get(item)))).then(updateStatusBarItems);
                    }
                    break;
                case 'Stash':
                    stash_xtasks();
    ...
    context.subscriptions.push(vscode.commands.registerCommand('xtask.listtestcases', async () => {
        ...
        quickPick.onDidTriggerButton(async (e) => {
            switch (e.tooltip) {
                case 'New':
                    ...
                    (0, node_fs_1.writeFileSync)(INPUT_TEMPFILE, '');
        ...
        quickPick.onDidTriggerItemButton(async (e) => {
            ...
            switch (e.button.tooltip) {
                case 'Show Diff':
                    (0, node_fs_1.writeFileSync)(EXPECT_TEMPFILE, e.item.test.expect);
                    (0, node_fs_1.writeFileSync)(ACTUAL_TEMPFILE, e.item.test.actual);
                    ...
                    break;
                case 'Edit Input':
                    (0, node_fs_1.writeFileSync)(INPUT_TEMPFILE, e.item.test.input);
                    await vscode.workspace.openTextDocument(INPUT_TEMPFILE).then(doc => { vscode.window.showTextDocument(doc); });
                    break;
                case 'Edit Expect':
                    (0, node_fs_1.writeFileSync)(EXPECT_TEMPFILE, e.item.test.expect);
